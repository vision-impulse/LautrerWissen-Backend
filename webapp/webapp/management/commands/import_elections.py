...
class CSVElectionResultRow:
            ...
            if old_key.startswith('D') or old_key.startswith('F'):
                try:
                    ...
                    if 0 <= index_from_key < len(PARTIES):
                        ...
                        print(f"Warning: Key '{old_key}' has an index ({index_from_key}) out of bounds for mapping_list.")
                        mapped_dict[old_key] = value # Keep original key if not found in mapping
                except ValueError:
    ...
    def get_stats_direct_votes(self):
        columns = {'Wahlberechtigte gesamt (A)': "Wahlberechtigte gesamt", 
    ...
    def get_result_table(self):
        ...
        for key in all_unique_keys:
            row = {
                'Name': key,
                'Direktstimme': direct_votes.get(key, None), # Use .get() with a default of np.nan for missing keys
        ...
        for key in all_unique_keys:
            row = {
                'Name': key,
                'Direktstimme': direct_stats.get(key, None), # Use .get() with a default of np.nan for missing keys
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        csv_file = os.path.join(os.getenv("APP_DATA_DIR"), "initial/data/bundestagswahl_2025.csv")
        ...
        self.import_restult_for_district_type(df, district_type="GEMEINDE", election=election)
        self.import_restult_for_district_type(df, district_type="STADTTEIL", election=election)
        self.import_restult_for_district_type(df, district_type="BRIEFWAHLBEZIRK", election=election)
