...
class StolpersteinTableExtractor(DefaultWikipediaTableExtractor):

    def __init__(self):
        table_structure = [("image", WikipediaTableCellType.IMAGE),
                           ("name", WikipediaTableCellType.TEXT),
                           ("address", WikipediaTableCellType.ADDRESS),
                           ("timespan", WikipediaTableCellType.TEXT),
                           ("description", WikipediaTableCellType.TEXT)]
        super(StolpersteinTableExtractor, self).__init__(table_structure)
            ...
            if link and 'href' in link.attrs:
                if self.geo_keyword in link['href']:
                    if self.location_and_gps_originate_from_one_cell:
                        row_data.append(cell_text[:-6].strip())  # split location and gps in two separate columns
                    else:
